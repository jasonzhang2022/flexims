...
function fxPickListDest($compile, $templateCache){
	...
	return {
		...
		controller: function($scope, $element, $attrs){
			...
			var ngOption=$scope.ngOptions.replace($scope.srcoptionsExp, "destoptions").replace(/^.+\s+as\s+/, "");
			if ($scope.groupByStr){
				//group by interfere with ordering
				var reg=new RegExp("\\s+group\\s+by\\s+"+$scope.groupByStr); 
				ngOption=ngOption.replace(reg, "");
			}
			$attrs.ngOptions=ngOption;
